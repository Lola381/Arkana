...
class MetricsLogger:
    """
    Logs query metrics for observability dashboard.
    """
    ...
    async def log_query(
        ...
        """Log a query for metrics aggregation"""
        record = {
            "timestamp": datetime.utcnow().isoformat(),
            "query": query,
    ...
    async def flush(self):
        ...
        if not self.buffer or not self.db_pool:
            return
        ...
        try:
            async with self.db_pool.acquire() as conn:
                await conn.executemany("""
                    ...
                    (timestamp, query, latency_ms, chunks_retrieved, 
                     faithfulness_score, relevance_score, user_region, refused)
                    VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
                """, [
                    (r["timestamp"], r["query"], r["latency_ms"], 
                     json.dumps(r["chunks_retrieved"]), r["faithfulness_score"],
                     r["relevance_score"], r["user_region"], r["refused"])
                    for r in records
                ])
        except Exception as e:
