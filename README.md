...
<p align="center">
  <img src="arkana-react/public/favicon.svg" width="80" alt="ARKANA Logo"/>
</p>
...
* **Robust Auth & Security**: Dual-backend support (Node.js/Express & Python/Flask) with JWT authentication and secure MongoDB Atlas integration.
...
```
SPD2/
├── arkana-react/                  # Primary React 19 SPA & Backend Application
...
│   └── package.json
├── ai/                            # AI/RAG Data Pipeline & Ingestion Submodule
│   └── ai/
│       ├── backend/               # Python ingestion engine (Wikidata, UNESCO, OSM)
│       ├── chunking/              # Semantic chunking pipeline
│       ├── embedding/             # Dense vector embedding generators
│       └── retrieval/             # RRF (Reciprocal Rank Fusion) hybrid search
├── arkana/                        # Legacy Static HTML/JS Prototype
├── ARKANA_DEVELOPER_PRESENTATION.md # Developer technical architecture document
├── CONTEXT.md                     # Project history & session documentation
...
* **Python** 3.10+ *(if running the Python Flask backend option)*
...
```bash
cd arkana-react
python -m pip install flask flask-cors pymongo certifi pyjwt python-dotenv werkzeug
python server.py
# Output: Connected to MongoDB Atlas. Database: arkana
...
* **Strengths**: Clean separation of concerns, polished photography-style animations, dual-backend flexibility, responsive layout, zero broken links or missing assets.
